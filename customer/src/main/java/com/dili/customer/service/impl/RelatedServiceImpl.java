...
public class RelatedServiceImpl extends BaseServiceImpl<Related, Long> implements RelatedService {
    ...
    @Override
    public PageInfo<Related> queryAll(RelatedQuery query){
        PageHelper.startPage(query.getPageNum(), query.getPageSize());
        List<Related> list = relatedMapper.selectByQuery(query);
        PageInfo<Related> pageInfo = new PageInfo<>(list);
        return pageInfo;
    }
    ...
    public void deleteAll(Long[] ids) {
        for (Long id : ids) {
            relatedMapper.deleteByPrimaryKey(id);
        }
    }
